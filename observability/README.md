...
This layer makes every run of the agent workflow **observable**: each turn is captured as a structured trace in [LangSmith](https://smith.langchain.com), with PII masked, organised by conversation and turn, and annotated with the turn's outcome. It is **backstage** — for *developers*.
...
1. **Create a free LangSmith account** → [https://smith.langchain.com](https://smith.langchain.com) (Developer tier: 1 seat, ~5k traces/month — verify current limits). **Settings → API Keys → Create**.
