...
- **Prompt:** `prompts/output.py` · v1.7.0 (general + analytics + **advice** modes; analytics is **answer-first** — direct yes/no + data scope before the number — and multi-row → table with complaint + reporter/assignee + **Status** columns; advice = grounded safety-first guidance across **all models** — one shared answer + per-model deltas).
